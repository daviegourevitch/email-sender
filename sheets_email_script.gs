...
// Marcels function
function sendAllEmailsSheets() {
  var sheet = SpreadsheetApp.openByUrl('https://docs.google.com/spreadsheets/d/1MvBx2cORCNMIZ-VUjnzu7DDu-av_lfJZg_p8kYb4Ayw/edit#gid=1960322623');
  ...
  for (i = 0; i < values.length; i++) {
    emails.push(__generateFullEmail(values[i]));
  }
  emails.forEach(__sendEmail);
}

function sendAllEmailsSheetsCSV() {
  var sheet = SpreadsheetApp.openByUrl('https://docs.google.com/spreadsheets/d/1MvBx2cORCNMIZ-VUjnzu7DDu-av_lfJZg_p8kYb4Ayw/edit#gid=1960322623');
  var range = sheet.getDataRange();
  var values = range.getValues();
  
  var emailObj = __generateFullEmail(values[0]);
  emailObj.csv = values.join(',');
  sendMassEmail(emailObj);
}

function __generateFullEmail(emailAddress) {
...
function __createEmails(response) {
  ...
  if (itemResponses[1]) {
    ...
    */
    ...
    if (emailChoiceStrings[3]) { emailChoiceBools = [true, true, true, true]; }
  } else {
...
function sendMassEmail(emailObj) {
  try {
    GmailApp.sendEmail(
      emailObj.emailAddress,
      'Welcome to CUSC Gaming!',
...
function testSendAllEmailsAsDrafts() {
  var form = FormApp.openByUrl(
    'https://docs.google.com/forms/d/1nMYUm562PXBXwo7rNa9jOGX2scaiQC8QW0dsgF5TG9Y/edit'
   );
  ...
  var responses = form.getResponses();
  var composedEmails = responses.map(__createEmails);
  composedEmails.forEach(__sendAsDraft);

}

function __sendAsDraft(emailObj) {
  try {
...
function testSingleEmailGeneration() {
  var form = FormApp.openByUrl(
    'https://docs.google.com/forms/d/1nMYUm562PXBXwo7rNa9jOGX2scaiQC8QW0dsgF5TG9Y/edit'
   );
  ...
  var emailObj = __createEmails(responses[6]);
